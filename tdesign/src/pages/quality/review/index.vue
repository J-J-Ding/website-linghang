    ...
    <t-space direction="vertical">
      <t-space>
        ...
        <t-input placeholder="Search Input" clearable>
          <template #suffixIcon>
            <search-icon :style="{ cursor: 'pointer' }" @search="handleSearch" />
          </template>
        </t-input>
      </t-space>
      ...
      >
        <t-tooltip content="新建对话" placement="top-left">
          <t-button class="review-new-chat" size="large" theme="primary" shape="circle" variant="base" @click="newChat">
            <icon name="plus" />
          </t-button>
        </t-tooltip>
        <template #header>
...
import { SystemSumIcon, Icon, ArrowDownIcon, SearchIcon } from 'tdesign-icons-vue-next';
...
const columns = [
  { colKey: 'id', title: '标识', width: '120' },
  { colKey: 'title', title: '主题', width: '500' },
  { colKey: 'domain', title: '领域', width: '80', edit: { component: Input, showEditIcon: false } },
  { colKey: 'team', title: '团队', width: '200', edit: { component: Input, showEditIcon: false } },
  { colKey: 'person', title: '引入人', width: '120', edit: { component: Input, showEditIcon: false } },
  {
    colKey: 'ai',
    title: '智能体',
    width: '80',
    ...
    cell: (h, { row }) => {
      return (
        <t-button
          ...
          size="small"
          onClick={() => {
            componentSelect.value = row.componentName;
            console.log('AI助手 - 当前组件:', componentSelect.value);
...
const onTableRead = async () => {
  ...
  try {
    ...
    let response;
    try {
      response = await fetch(`${SERVER_API_URL}/api_data/Rdc_list_read`, {
        method: 'GET',
    ...
    // 4. 检查响应状态（假设接口返回 { status: 'success', data: [...] }）
    if (responseData.status !== 'success') {
      const errorMsg = responseData.message || '读取列表失败';
    ...
    // 5. 提取并处理表格数据
    let tableData = responseData.result;
    ...
    try {
      ...
      data.value = tableData.map((item) => ({
        id: item['标识']?.toString() || '',
        title: item['主题']?.toString() || '',
        domain: item['领域']?.toString() || '',
        team: item['团队']?.toString() || '',
        person: item['引入人']?.toString() || '',
      }));
      ...
      return tableData; // 可选：返回数据供其他逻辑使用
    } catch (error) {
