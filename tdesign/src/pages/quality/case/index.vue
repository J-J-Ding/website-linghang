    ...
    <div class="scene-main">
        <t-space direction="vertical">
            <t-space>
                ...
                placeholder="请选择案例ID"
                label="案例ID: ">
                    <t-option v-for="option in case_id_option" :key="option" :value="option" :label="option" />
                </t-select>
                <t-select
                v-model="work_item_type_list"
                multiple
                filterable
                clearable
                :min-collapsed-num="1"
                ...
                label="工作项类型: ">
                    <t-option v-for="option in work_item_type_option" :key="option" :value="option" :label="option" />
                </t-select>
                <t-input 
                v-model="work_item_id_list" 
                ...
                placeholder="请选择状态"
                label="状态: ">
                    <t-option v-for="option in status_option" :key="option" :value="option" :label="option" />
                </t-select>
              </t-space>
               <t-space>
                <t-input 
                v-model="assignee_list" 
                placeholder="请输入指派人" 
                clearable
                style="width: 300px;"
                label="指派人: "
                />
                <t-input 
                v-model="horizontal_push_ticket_list" 
                placeholder="请输入横推单" 
                clearable
                style="width: 300px;"
                label="横推单: "
                ...
                placeholder="请选择案例分类"
                label="案例分类: ">
                    <t-option v-for="option in case_category_option" :key="option" :value="option" :label="option" />
                </t-select>
               </t-space>
               <t-space>
                <t-input 
                v-model="performance_category_list" 
                placeholder="请输入性能分类" 
                clearable
                style="width: 300px;"
                label="性能分类: "
                />
                <t-input 
                v-model="problem_description_list" 
                placeholder="请输入问题描述" 
                clearable
                style="width: 300px;"
                label="问题描述: "
                ...
                />
                <t-input 
                v-model="trigger_first_office_list" 
                placeholder="请输入触发首局" 
                clearable
                style="width: 300px;"
                label="触发首局: "
                />
                <t-input 
                v-model="closure_responsible_person_list" 
                placeholder="请输入闭环责任人" 
                clearable
                style="width: 300px;"
                label="闭环责任人: "
                />
               ...
               <t-space>
                <t-select
                v-model="affect_business_flag_list"
                multiple
                filterable
                clearable
                :min-collapsed-num="1"
                ...
                label="是否影响业务: ">
                    <t-option v-for="option in affect_business_flag_option" :key="option" :value="option" :label="option" />
                </t-select>
                <t-input 
                v-model="introduction_point_root_cause_list" 
                placeholder="请输入引入点根因分类" 
                clearable
                style="width: 300px;"
                label="引入点根因分类: "
                />
                <t-input 
                v-model="feature_list" 
                placeholder="请输入特性" 
                clearable
                style="width: 300px;"
                label="特性: "
              ...
              <t-space>
                ...
                <t-button disabled @click="filterData">筛选</t-button>
                <t-button disabled @click="resetFilters">重置</t-button>
                <t-button disabled @click="openAddDialog">新增</t-button>
                <t-button disabled @click="onBatchDelete">批量删除</t-button>
                <t-button disabled @click="onBatchImport">批量导入</t-button>
                <t-button disabled @click="onBatchOutput">批量导出</t-button>
              </t-space>
            <p> </p>
          </t-space>
        <t-table
        ...
        theme="warning">
            ...
            <div class="add-form">
                <t-form ref="addFormRef" :data="dialogFormData" layout="vertical" label-width="170px">
                    <t-form-item label="* 网元业务方案" name="netBusinessScheme">
                        <t-select
                        v-model="dialogFormData.netBusinessScheme"
                        placeholder="请选择网元业务方案"
                        creatable
                        filterable
                        :disabled="false"
        ...
        theme="info">
            ...
            <div class="query-form">
                <t-form :data="dialogNetFormData" layout="vertical" label-width="170px">
                    ...
                    <t-form-item label="关联版本号" name="related_version">
                        <t-input v-model="dialogNetFormData.related_version" :readonly="true" autosize/>
                    </t-form-item>
                    ...
                    <t-form-item label="数据状态" name="status">
                        ...
                        <t-tag :theme="dialogNetFormData.status === '已闭环' ? 'success' : 'danger'" size="small">{{ dialogNetFormData.status }}</t-tag>
                    </t-form-item>
                </t-form>
            </div>
            <template #footer>
...
import { queryNetBusinessByParams, queryNetBusinessTree, queryNetBusinessStatusList, queryNetBusinessNetBusinessSchemeList, queryNetBusinessCrossTypeList,
    queryNetBusinessBoardBusinessModelList,queryNetBusinessNetBusinessModelList, queryNetBusinessBusinessTypeList, addNetBusinessData, updateNetBusinessData,
         deleteNetBusinessData, importExcelbaseNetBusinessData, queryCaseLibraryByParams } from '@/api/electric.js';
...
const tableColumnList = [
    {
        ...
        type: 'multiple',
        width: '50',
        checkProps: ({row}) => ({ disabled: row.status !== '已闭环', title: row.status !== '已闭环' ? '不可选' : null }),
    },
    // { colKey: 'index', title: '编号', width: '70', align: 'center' },
    { colKey: 'case_id', title: '案例ID', width: '100', align: 'center' },
    { colKey: 'work_item_type', title: () => (<div style={{ textAlign: 'center' }}>工作项类型</div>), width: '150', align: 'center', ellipsis: { theme: 'light', placement: 'bottom',} },
    { colKey: 'work_item_id', title: () => (<div style={{ textAlign: 'center' }}>工作项ID</div>), width: '200', align: 'center', ellipsis: { theme: 'light', placement: 'bottom',} },
    { colKey: 'work_item_title', title: () => (<div style={{ textAlign: 'center' }}>工作项标题</div>), width: '200', align: 'left', ellipsis: { theme: 'light', placement: 'bottom',} },
    {
        colKey: 'modify_fault_record',
        title: '修改故障入库记录',
        width: '200',
        align: 'center',
        ellipsis: { theme: 'light', placement: 'bottom',},
        cell: (_, { row }) => (<t-link theme="primary" href={row.modify_fault_record} target="_blank" hover="underline">{row.modify_fault_record}</t-link>)
    },
    {
        colKey: 'introduce_fault_record',
        title: '引入故障入库记录',
        width: '200',
        align: 'center',
        ellipsis: { theme: 'light', placement: 'bottom',},
        cell: (_, { row }) => (<t-link theme="primary" href={row.introduce_fault_record} target="_blank" hover="underline">{row.introduce_fault_record}</t-link>)
    },
    { colKey: 'fault_branch', title: () => (<div style={{ textAlign: 'center' }}>故障分支</div>), width: '200', align: 'center', ellipsis: { theme: 'light', placement: 'bottom',} },
    { 
        colKey: 'key_fault_code', 
        title: () => (<div style={{ textAlign: 'center' }}>关键故障代码</div>), 
        width: '200', 
        align: 'left', 
        ellipsis: { theme: 'light', placement: 'bottom',},
        cell: (h, { row }) => {
          return h('div', {
            domProps: {
              innerHTML: row.key_fault_code // 确保此HTML内容是安全的
            },
            style: {
              textAlign: 'left'
            }
    ...
    { colKey: 'modified_code', title: () => (<div style={{ textAlign: 'center' }}>修改后代码</div>), width: '200', align: 'left', ellipsis: { theme: 'light', placement: 'bottom',}},
    { colKey: 'assignee', title: '指派人', width: '200', align: 'center'},
    { colKey: 'horizontal_push_ticket', title: '横推单', width: '200', align: 'center'},
    { colKey: 'notice_ticket', title: '通告单', width: '200', align: 'center'},
    { colKey: 'review_ticket', title: '复盘单', width: '200', align: 'center'},
    { colKey: 'case_category', title: '案例分类', width: '200', align: 'center'},
    { colKey: 'performance_category', title: '性能分类', width: '200', align: 'center'},
    { colKey: 'create_time', title: '创建时间', width: '200', align: 'center'},
    { colKey: 'completion_time', title: '完成时间', width: '200', align: 'center'},
    { colKey: 'problem_description', title: () => (<div style={{ textAlign: 'center' }}>问题描述</div>), width: '300', align: 'center', ellipsis: { theme: 'light', placement: 'bottom',} },
    { colKey: 'belong_project', title: '归属项目', width: '200', align: 'center'},
    { colKey: 'occurrence_time', title: '发生时间', width: '200', align: 'center'},
    { colKey: 'trigger_first_office', title: () => (<div style={{ textAlign: 'center' }}>触发首局</div>), width: '200', align: 'center', ellipsis: { theme: 'light', placement: 'bottom',} },
    { colKey: 'closure_time', title: '闭环时间', width: '200', align: 'center'},
    { colKey: 'closure_responsible_person', title: '闭环责任人', width: '200', align: 'center'},
    { colKey: 'affect_business_flag', title: '是否影响业务', width: '200', align: 'center'},
    { colKey: 'root_cause', title: () => (<div style={{ textAlign: 'center' }}>问题根因</div>), width: '200', align: 'left', ellipsis: { theme: 'light', placement: 'bottom',}},
    { colKey: 'emergency_method', title: () => (<div style={{ textAlign: 'center' }}>应急方法</div>), width: '200', align: 'left', ellipsis: { theme: 'light', placement: 'bottom',}},
    { colKey: 'inspection_method', title: () => (<div style={{ textAlign: 'center' }}>巡检方法</div>), width: '200', align: 'left', ellipsis: { theme: 'light', placement: 'bottom',}},
    { colKey: 'avoidance_method', title: () => (<div style={{ textAlign: 'center' }}>规避方法</div>), width: '200', align: 'left', ellipsis: { theme: 'light', placement: 'bottom',}},
    { colKey: 'solution_version', title: '解决版本', width: '150', align: 'center'},
    { colKey: 'version_time', title: '版本时间', width: '200', align: 'center'},
    {
        colKey: 'code_document_link',
        title: '代码/文档链接',
        width: '200',
        align: 'center',
        ellipsis: { theme: 'light', placement: 'bottom',},
        cell: (_, { row }) => (<t-link theme="primary" href={row.code_document_link} target="_blank" hover="underline">{row.code_document_link}</t-link>)
    },
    { colKey: 'problem_code_annotation', title: () => (<div style={{ textAlign: 'center' }}>问题代码标注</div>), width: '200', align: 'left', ellipsis: { theme: 'light', placement: 'bottom',}},
    {
        colKey: 'self_test_record',
        title: '自测记录',
        width: '200',
        align: 'center',
        ellipsis: { theme: 'light', placement: 'bottom',},
        cell: (_, { row }) => (<t-link theme="primary" href={row.self_test_record} target="_blank" hover="underline">{row.self_test_record}</t-link>)
    },
    {
        colKey: 'system_test_record',
        title: '系测记录',
        width: '200',
        align: 'center',
        ellipsis: { theme: 'light', placement: 'bottom',},
        cell: (_, { row }) => (<t-link theme="primary" href={row.system_test_record} target="_blank" hover="underline">{row.system_test_record}</t-link>)
    },
    { colKey: 'introduction_point_root_cause', title: '引入点根因分类', width: '150', align: 'center' },
    { colKey: 'feature', title: '特性', width: '200', align: 'center' },
    { colKey: 'component', title: '组件', width: '150', align: 'center' },
    { colKey: 'board_part', title: '单板部件', width: '150', align: 'center' },
    { colKey: 'update_time', title: '更新时间', width: '200', align: 'center' },
    {
        ...
        align: 'center',
        fixed: 'right',
        cell: (_, { row }) => (<t-tag theme={row.status === '已闭环' ? 'success' : 'danger'} size="small">{row.status}</t-tag>)
    },
    // {
    //     colKey: 'edit',
    //     title: '操作',
    ...
    //     cell: (_, { row }) => (
    //         <>
    //         <t-link theme="primary" disabled hover="color" onClick={() => openQueryDialog(row)} style={{ marginRight: '8px' }}>查看</t-link>
    //         {(row.status === '已闭环' || row.status === '') ? (<t-link theme="primary" disabled hover="color" onClick={() => openEditDialog(row)}>修改</t-link>):null}
    //         </>
    //     )
    // }
];
...
const case_id_list = ref([]);
const case_id_option = ref([]);
const work_item_type_list = ref([]);
const work_item_type_option = ref(['变更请求','外场故障','待办事项']);
const work_item_id_list = ref('');
const work_item_title_list = ref('');
const status_list = ref(['新建']);
const status_option = ref(['新建','分析中','审批','已闭环']);
const assignee_list = ref('');
const horizontal_push_ticket_list = ref('');
const notice_ticket_list = ref('');
const review_ticket_list = ref('');
const case_category_list = ref([]);
const case_category_option = ref(['关键','严重','一般']);
const performance_category_list = ref('');
const problem_description_list = ref('');
const belong_project_list = ref('');
const trigger_first_office_list = ref('');
const closure_responsible_person_list = ref('');
const affect_business_flag_list = ref([]);
const affect_business_flag_option = ref(['是','否']);
const introduction_point_root_cause_list = ref('');
const feature_list = ref('');
const component_list = ref('');
const board_part_list = ref('');
...
const handleEditConfirm = async () => {
    if (!dialogFormData.value.netBusinessScheme) {
        MessagePlugin.warning('请选择或创建网元业务方案');
        return;
    }
