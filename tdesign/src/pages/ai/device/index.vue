...
const columns = [
  ...
  { colKey: 'l2svc', title: '业务类型', width: '180', edit: { component: Input, showEditIcon: false } },
  { colKey: 'otnsvc', title: '承载类型', edit: { component: Input, showEditIcon: false } },
  { colKey: 'uniprotect', title: '客户侧保护', edit: { component: Input, showEditIcon: false } },
  { colKey: 'nniprotect', title: '网络侧保护', edit: { component: Input, showEditIcon: false } },
  {
    colKey: 'edit',
    title: '操作',
    width: '120',
  ...
  try {
    ...
    try {
      ...
      data.value = tableData.map((item) => ({
        ...
        l2svc: item['业务类型']?.toString() || '',
        otnsvc: item['承载类型']?.toString() || '',
        uniprotect: item['客户侧保护']?.toString() || '',
        nniprotect: item['网络侧保护']?.toString() || '',
      }));
