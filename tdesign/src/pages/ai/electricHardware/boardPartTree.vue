            ...
            <div class="import-container">
                ...
                <!-- 添加 wrap="false" 禁止换行，确保在一行显示 -->
                    <t-space wrap="false" class="example-space">
                        ...
                        variant="text"
                        theme="primary"
                        @click="pubDownloadExampleFile('/templates/electric_hardware_board_part_tree_example.xlsx', '单板部件树批量导入示例.xlsx')"
                        class="example-btn">
...
import { queryBoardPartTreeByParams, queryBoardPartTreeTree, queryBoardPartTreeStatusList, queryBoardPartTreePartList,
         queryBoardPartTreeBusinessSchemeList, queryBoardPartTreeSchemeSliceList, addBoardPartTreeData, updateBoardPartTreeData,
         deleteBoardPartTreeData, importExcelBoardPartTreeData} from '@/api/electric.js';
...
const tableColumnList = [
    ...
    { colKey: 'part', title: () => (<div style={{ textAlign: 'center' }}>部件</div>), width: '150', align: 'left', ellipsis: { theme: 'light', placement: 'bottom',} },
...
const filterData = async () => {
    try {
        ...
        // 发送请求
        const queryBoardPartTreeByParamsResponse = await queryBoardPartTreeByParams(requestParams);
        tableDataList.value = queryBoardPartTreeByParamsResponse.data || [];
...
const refreshData = async () => {
    ...
    const [
        queryBoardPartTreeTreeResponse,
        queryBoardPartTreeByParamsResponse,
        queryBoardPartTreeStatusListResponse,
        queryBoardPartTreePartListResponse,
    ] = await Promise.all([
        queryBoardPartTreeTree(),
        queryBoardPartTreeByParams(),
        queryBoardPartTreeStatusList(),
        queryBoardPartTreePartList(),
    ]);

    partBusinessSchemeTreeList.value = pubBuildTreeWithText(queryBoardPartTreeTreeResponse.data || []);
    tableDataList.value = queryBoardPartTreeByParamsResponse.data || [];
    statusOptionList.value = queryBoardPartTreeStatusListResponse.data || [];
    allPartOptionList.value = queryBoardPartTreePartListResponse.data || [];
}
...
const openEditDialog = async (row) => {
    ...
    let queryBoardPartTreeBusinessSchemeListResponse = await queryBoardPartTreeBusinessSchemeList({"part": row.part});
    allBusinessSchemeOptionList.value = queryBoardPartTreeBusinessSchemeListResponse.data || [];

    // 查询业务方案切片列表
    let queryBoardPartTreeSchemeSliceListResponse = await queryBoardPartTreeSchemeSliceList({"businessScheme": dialogFormData.value.businessScheme});
    allSchemeSliceOptionList.value = queryBoardPartTreeSchemeSliceListResponse.data || [];
...
const handleEditConfirm = async () => {
    ...
    try {
        let updateBoardPartTreeDataResponse = await updateBoardPartTreeData(updatedData);

        await refreshData();

        editDialogVisible.value = false;
        MessagePlugin.success(updateBoardPartTreeDataResponse.message);
    } catch (error) {
...
const handleAddConfirm = async () => {
    ...
    try {
        let addBoardPartTreeDataResponse = await addBoardPartTreeData(newRow);

        await refreshData();

        addDialogVisible.value = false;
        MessagePlugin.success(addBoardPartTreeDataResponse.message);
    } catch (error) {
...
const handleDeleteConfirm = async () => {
    try {
        ...
        await deleteBoardPartTreeData({"id": deleteRowIds.value.join(',')});
...
const handleImportSubmit = async () => {
    ...
    try {
        ...
        // 调用后端批量导入接口
        const response = await importExcelBoardPartTreeData(formData);
...
const onBatchOutput = async () => {
    ...
    try {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, '单板部件树批量导出');
        ...
        const fileName = `单板部件树批量导出_${formattedDate}.xlsx`;
        // 导出文件
...
const handlePartChange = async (value) => {
    ...
    let queryBoardPartTreeBusinessSchemeListResponse = await queryBoardPartTreeBusinessSchemeList({"part": value});
    allBusinessSchemeOptionList.value = queryBoardPartTreeBusinessSchemeListResponse.data || [];
