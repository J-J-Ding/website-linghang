          ...
          <t-space direction="vertical" style="width: 100%">
            ...
            <div class="prompt-filter-container">
              <t-button
                ...
                :key="filter.value"
                :variant="selectedFilter === filter.value ? 'outline' : 'base'"
                theme="default"
                shape="round"
                size="medium"
                    ...
                    <template #actions>
                      ...
                      <a
                        ...
                        :style="{
                          ...
                          color: template.creator === (user.userInfo.name || '') ? '#409eff' : '#c0c0c0',
                          'font-size': '14px',
                          'margin-right': '10px',
                          'pointer-events': template.creator === (user.userInfo.name || '') ? 'auto' : 'none',
                        }"
                      >
                        修改
                      </a>
...
const selectAgent = [
  {
    label: '问答模式',
    value: 'BasicChatAgent',
  },
  {
    label: '超级问答模式',
    value: 'SuperChatAgent',
  },
  {
    label: '详设评审智能体',
...
const selectedFilter = ref('all'); // 默认选择"全部"
...
const filteredTemplates = computed(() => {
  let result = Array.isArray(templates.value) ? templates.value : [];

  // 首先应用筛选条件
  if (selectedFilter.value === 'mine') {
    // 包含user.userInfo.name的模板
    result = result.filter((template) => template.creator && template.creator.includes(user.userInfo.name));
  } else if (selectedFilter.value === 'current') {
    // 根据aiChatTitle进行筛选
    result = result.filter((template) => template.scene && template.scene.includes(props.aiChatTitle));
  } else if (selectedFilter.value === 'popular') {
    // 引用次数超过20次的为热门
    result = result.filter((template) => (template.reference_count || 0) > 20);
  } else if (selectedFilter.value === 'general') {
    // 假设有某种方式标记通用模板，这里暂时按场景包含"通用"关键词筛选
    result = result.filter(
      (template) =>
        template.scene && (template.scene.includes('通用') || template.scene.toLowerCase().includes('general')),
    );
  } else if (selectedFilter.value === 'favorite') {
    // 假设后端会在模板中添加收藏标记，这里暂时筛选标记为收藏的模板
    result = result.filter((template) => template.is_favorite || template.favorite);
  }
