            ...
            >
              <template #prefix>
                <div class="m-chat-select">
                  ...
                  <t-select v-model="selectAgentModelValue" :options="selectAgentModel" value-type="object"></t-select>
                </div>
              </template>
            </t-chat-sender>
          </template>
...
const selectAgentModel = ref([]);
const selectAgentModelValue = ref({});


const AgentOptions = {
  'BusinessCodeGen': '业务代码生成',
  'UTGen': 'UT生成',
  'FTGen': 'FT生成',
  'ZxsemCodeGen': '巡检工具代码生成',
}
...
const handleData = async (inputValue) => {
  ...
  try {
    ...
    var content = JSON.parse(props.aiChatContext);
    var data = {};

    if(content.AI应用平台 == "DN"){
      data = {
        user: user.userInfo.name,
        session_id,
        request_id: uuidv4(),
  ...
  (newContent) => {
    if (newContent.length > 0) {
      ...
      if(!Array.isArray(content)){
        const typekey = content.AI应用类型;
        if (AgentOptions.hasOwnProperty(typekey)) {
          selectAgent.value = [{ label: AgentOptions[typekey], value: typekey}];
          selectAgentValue.value = { label:AgentOptions[typekey], value: typekey };
          // 加载模型
          selectAgentModel.value = [{ label: content.AI应用名称, value: content.AI应用名称}];
          selectAgentModelValue.value = { label: content.AI应用名称, value: content.AI应用名称};
        }else{
          selectAgent.value = [];
          selectAgentValue.value = {};

          selectAgentModel.value = [];
          selectAgentModelValue.value = {};
        }
      }
    }
  },
