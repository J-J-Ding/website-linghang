      ...
      <div class="time-range-selector">
        ...
        <div class="time-range-header">
          <div class="preset-buttons">
            <t-button variant="outline" size="small" @click="setTimeRange('today', true)">今天</t-button>
            <t-button variant="outline" size="small" @click="setTimeRange('yesterday', true)">昨天</t-button>
            <t-button variant="outline" size="small" @click="setTimeRange('last7days', true)">最近7天</t-button>
            <t-button variant="outline" size="small" @click="setTimeRange('last30days', true)">最近30天</t-button>
          </div>
        </div>
        ...
        <div class="time-range-picker">
          ...
          <div class="oms-display">
            ...
            @popup-visible-change="showSelectOMS">
              <t-option v-for="item in omsOptions" :key="item.id" :value="item.value" :label="item.label"></t-option>
            </t-select>
          </div>
      ...
      <div class="analysis-results-section">
        ...
        <div class="results-table-container">
          ...
          >
            ...
            <template #recoveryPath="{ row }">
              <div class="path-cell">
                <div class="path-text" :title="row.recoveryPath">
                  {{ row.recoveryPath }}
                </div>
            ...
            <div class="business-items">
              ...
              >
                <div class="business-info">
                  ...
                  <div class="business-path">{{ getPathPreview(business.affected_oms || business.oms_dict_list) }}</div>
                </div>
                <t-button 
                  variant="text" 
                  size="small" 
        ...
        <div class="business-info-section">
          ...
          <div class="info-grid">
            ...
            <div class="info-item">
              ...
              <span class="info-value">{{ selectedBusiness.conn_attr }}</span>
            </div>
            <div class="info-item">
        ...
        <div class="path-info-section">
          ...
          <div class="path-display">
            <div class="path-nodes">
              <span 
                v-for="(node, index) in getPathNodes(selectedBusiness.recoveryPath)" 
                :key="index"
                :class="['path-node', { 'alert-node': isAlertNode(node) }]"
              >
                {{ node }}
                <span v-if="index < getPathNodes(selectedBusiness.recoveryPath).length - 1" class="path-arrow">→</span>
              </span>
            </div>
...
import { getHistoricalList, getProjectList, searchOMS, topologyDataList } from '@/api/analysis';
...
const setTimeRange = (rangeType, showMessage = true) => {
  const now = new Date();
...
const showSelectOMS = async () => {
  if (keyWord.value && keyWord.value.trim()) {
    omsOptions.value = await getOMSData(projectName.value, keyWord.value);
  } else {
    const { oms_dict_list } = route.query;
    if (oms_dict_list) {
      omsOptions.value = JSON.parse(oms_dict_list);
    }
  }
};

const getOMSData = async (name, word) => {
  try {
    const params = {
      project_name: name,
      keyword: word,
    };
    const response = await searchOMS(params);
...
const getHistoricalTable = async (project_name, enable_limit, page, limit, stime, etime, oms_lists) => {
  try {
    const params = {
    ...
    const response = await getHistoricalList(params);
    
    // 为每行数据生成唯一ID
    businessList.value = (response.data || []).map((item, index) => ({
      ...item,
      id: `row_${index}_${Date.now()}` // 确保有唯一ID
    }));
...
const parseRouteParams = async () => {
  const { end_time, start_time, oms_lists, project_name, oms_dict_list } = route.query;
  
  if (oms_lists) {
    OMSList.value = JSON.parse(oms_lists);
  }
  ...
  if (start_time) {
    try {
      ...
      if (!isNaN(endDate.getTime())) {
        ...
        await getHistoricalTable(projectName.value, 1, pagination.defaultCurrent, pagination.defaultPageSize, formattedTimeRange.value[0], formattedTimeRange.value[1], OMSList.value);
...
const handleAnalyze = async () => {
  ...
  await getHistoricalTable(projectName.value, 1, 1, pagination.defaultPageSize, formattedTimeRange.value[0], formattedTimeRange.value[1], omsValue.value);
...
const exportAlarmTable = async (project_name, enable_limit, page, limit, stime, etime, oms_lists) => {
  try {
    const params = {
        project_name: project_name,
        enable_limit: enable_limit,
...
const exportResults = async () => {
  try {
    const tableData = await exportAlarmTable(projectName.value, 0, 1, pagination.defaultPageSize, formattedTimeRange.value[0], formattedTimeRange.value[1], OMSList.value);
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, '历史动作的业务列数据')
    ...
    const fileName = `历史动作的业务列表_${formattedDate}.xlsx`
...
const handlePageChange = async (pageInfo) => {
  await getHistoricalTable(projectName.value, 1, pageInfo.current, pageInfo.pageSize, formattedTimeRange.value[0], formattedTimeRange.value[1], OMSList.value);
...
// 修改 getPathPreview 函数，处理各种可能的数据类型
...
const getLinkColor = (link) => {
  ...
  if (hoveredcall_id.value) {
    if (link.businessId === hoveredcall_id.value) {
      if (link.type === 'shared') return '#b37feb'; // 悬停时共享路径高亮为浅紫色
...
const getLinkStrokeWidth = (link) => {
  ...
  if (link.type === 'shared') return 3.5; // 共享路径加粗显示
...
const updateTopologyData = () => {
  ...
  topologyLinks.value.forEach(link => {
    ...
    link.hasShared = false; // 是否有共享路径
    link.sharedBusinesses = []; // 共享路径的业务列表
  });
  ...
  // 先收集所有业务的链路信息，按业务分组
  const perBusinessLinks = new Map(); // 按业务分组存储链路
  ...
  // 遍历所有业务的链路
  perBusinessLinks.forEach((businessLinks, businessId) => {
    // 处理动作前路由
    businessLinks.oldRoutes.forEach((linkInfo) => {
      addLinkToMap(linkBusinessMap, linkInfo, 'old-route', businessId);
    });
    
    // 处理动作后路由
    businessLinks.newRoutes.forEach((linkInfo) => {
      addLinkToMap(linkBusinessMap, linkInfo, 'new-route', businessId);
    });
  });
  
  // 辅助函数：将链路添加到Map中
  function addLinkToMap(map, linkInfo, routeType, businessId) {
    const { sourceId, sourceName, targetId, targetName } = linkInfo;
    ...
    if (!map.has(linkKey)) {
      map.set(linkKey, {
        ...
        targetNode,
        businesses: [],
        oldRouteBusinesses: new Set(),
        newRouteBusinesses: new Set(),
        sharedBusinesses: new Set(),
        hasOldRoute: false,
        hasNewRoute: false,
        hasShared: false
      });
  ...
  // 检测共享路径：对于每个业务，检查是否在同一链路上同时有动作前和动作后路由
  linkBusinessMap.forEach((linkData, linkKey) => {
    const { businesses, oldRouteBusinesses, newRouteBusinesses } = linkData;
    
    // 遍历每个业务，检查是否同时有old-route和new-route
    for (const businessId of businesses) {
      if (oldRouteBusinesses.has(businessId) && newRouteBusinesses.has(businessId)) {
        linkData.sharedBusinesses.add(businessId);
        linkData.hasShared = true;
      }
    }
    
    // 如果有共享路径，记录到控制台
    if (linkData.hasShared) {
      console.log(`共享路径检测: ${linkData.sourceNode.name} -> ${linkData.targetNode.name}, 共享业务:`, Array.from(linkData.sharedBusinesses));
    }
  });
  
  // 更新链路类型和样式
  linkBusinessMap.forEach((linkData, linkKey) => {
    const { sourceNode, targetNode, businesses, hasOldRoute, hasNewRoute, hasShared, sharedBusinesses } = linkData;
    ...
    // 确定链路类型（优先级：shared > new-route > old-route > base）
    let linkType = 'base';
    if (hasShared) {
      linkType = 'shared';
    } else if (hasNewRoute) {
    ...
    if (existingLinks.length > 0) {
      existingLinks.forEach(link => {
        ...
        link.sharedBusinesses = Array.from(sharedBusinesses);
      });
    } else {
      // 创建虚拟链路
      ...
      const newLink = {
        ...
        sharedBusinesses: Array.from(sharedBusinesses)
      };
  ...
  console.log('链路类型分布:', 
    'shared(重合路径):', topologyLinks.value.filter(l => l.type === 'shared').length,
...
const applyRouteToLinks = (parsedLink, routeType, businessId) => {
  ...
  if (forwardLinks.length > 0) {
    ...
    // 【关键修复】不直接使用反向链路，而是创建一个新的正向虚拟链路
    // 这样可以保证箭头方向正确
    console.log(`创建新的正向链路: ${sourceNode.name} -> ${targetNode.name} (原链路方向相反)`);
    
    // 计算链路路径（从 sourceNode 到 targetNode）
    const dx = targetNode.x - sourceNode.x;
    const dy = targetNode.y - sourceNode.y;
    const dist = Math.sqrt(dx * dx + dy * dy);
    const midX = (sourceNode.x + targetNode.x) / 2;
    ...
    const controlX = midX;
    const controlY = midY - dist * 0.15; // 偏移一点，避免与原有链路重叠
