...
import { getHistoricalList, getFiberList, searchHOVAlert, getProjectList, topologyDataList } from '@/api/analysis';
...
const getHistoricalTable = async (project_name, alarm_id, page, limit, enable_limit) => {
  try {
    ...
    const response = await getFiberList(params);
...
const getLinkDashArray = (link) => {
  if (link.type === 'shared') return '10,6';
...
const updateTopologyData = () => {
  ...
  const allBusinessLinks = [];
  const perBusinessLinks = new Map();
  ...
  allBusinessLinks.forEach((linkInfo) => {
    ...
    if (!linkBusinessMap.has(linkKey)) {
      linkBusinessMap.set(linkKey, {
        ...
        targetNode,
        businesses: [],
        businessesWithOldRoute: new Set(),
        businessesWithNewRoute: new Set(),
        hasOldRoute: false,
        hasNewRoute: false,
        hasShared: false
      });
...
const getOMSData = async (name, starttime, endtime) => {
  try {
    ...
    const response = await searchHOVAlert(params);
...
const parseRouteParams = async () => {
  const { end_time, start_time, alarm_id, project_name } = route.query;
  if(alarm_id) {
    alarmValue.value.push(alarm_id);
  }
