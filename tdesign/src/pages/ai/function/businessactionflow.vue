...
import { getAnalysisList, searchOMSAlert, getProjectList, queryActionFlowList, queryActionFlowAllCall, topologyDataList } from '@/api/analysis';
import * as XLSX from 'xlsx';
import { useRoute } from 'vue-router'; // 导入useRoute
import { storage } from "@/utils/storage";

// 定义缓存key常量（与第一个组件保持一致）
...
const getHistoricalTable = async (project_name, call_id_list, page, limit, enable_limit, stime, etime) => {
  try {
      ...
      const response = await queryActionFlowList(params);
...
const getAllCall = async (name, starttime, endtime) => {
  try {
    ...
    const response = await queryActionFlowAllCall(params);
...
const exportResults = async () => {
  try {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, '日志分析业务动作流详情数据')
    ...
    const fileName = `日志分析业务动作流详情列表_${formattedDate}.xlsx`
...
const parseRouteParams = async () => {
    ...
    if(call_id) {
      ...
      await searchAllCall();
      omsValue.value = omsOptions.value;
    }
