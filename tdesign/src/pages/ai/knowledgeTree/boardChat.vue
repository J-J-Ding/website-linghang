...
const FPGAUrlAdd = (inputValue) => {
  const chatContext = JSON.parse(props.aiChatContext);
  const FPGA = chatContext.FPGA;

  // 匹配Markdown格式的超链接
  const linkPattern = /\[([^\]]+)\]\(([^)]+)\)/;
  const match = FPGA.match(linkPattern);

  if (match) {
    const [_, title, url] = match;
    
    // 检查输入内容是否包含标题
    if (inputValue.includes(title)) {
      // 在输入内容前添加URL
      inputValue = `${url}\n${inputValue}`;
    }
  }
...
const handleData = async (inputValue) => {
  ...
  inputValue = FPGAUrlAdd(inputValue);
