...
// 判断是否在子路由（版本列表或版本案例清单）
const isChildRoute = computed(() => {
    const path = route.path;
    // 判断路径是否包含 /version，且不是父路由本身
    return path.includes('/issueImpacte/version') && path !== '/ai/issueImpacte';
});
...
const handleAddConfirm = async () => {
    ...
    try {
        loading.value = true;
        // 确保所有字段都被包含，包括 assignTo
        const formData = { ...dialogFormData.value };
        // 确保 assignTo 字段存在（即使为空字符串）
        ...
        console.log('assignTo 字段值:', formData.assignTo);
