...
class IcenterAPI:
    ...
    def get_children(self, spaceId, contentId, depth=9):
        url = "https://icenterapi.zte.com.cn/zte-rdcloud-space-pluginoff/macro/execute?system=space-1920482552&environmentType=browser"
        body = {
            "macroKey": "ChildrenDisplayMacro",
            "macroData": {"spaceId": spaceId, "contentId": contentId, "depth": depth}
        }
        try:
            resp = requests.post(url, data=json.dumps(body), headers=self.headers, auth=self.auth, proxies=self.proxies)
        except Exception as e:
        ...
        try:
            if resp.status_code != 200:
                ...
                resp = requests.post(url, data=json.dumps(body), headers=self.headers, auth=self.auth, proxies=self.proxies)
            reps_json_data = json.loads(resp.content)
            if reps_json_data.get("code", {}).get("msgId") != 'RetCode.Success':
                return None
            all_nodes = reps_json_data.get('bo', [])
            if not all_nodes:
                return {}
            ...
            for node in all_nodes:
                ...
                node_url = f"https://i.zte.com.cn/index/ispace/#/space/{spaceId}/wiki/page/{nid}/view"
                nodes_map[nid] = {
                    "title": title,
                    "url": node_url,
                    "children": {}
            ...
            for node in all_nodes:
                ...
                if pid and pid in nodes_map:
                    nodes_map[pid]["children"][nid] = nodes_map[nid]
                elif nid == contentId:
                    root = nodes_map[nid]

            def build_markdown_tree(node):
                title = node.get("title", "")
                url = node.get("url", "")
                ...
                for child in children.values():
                    child_key, child_value = build_markdown_tree(child)
                    result[child_key] = child_value
                return key, result

            if root:
                _, result = build_markdown_tree(root)
                return result
            return {}
        except Exception as e:
            logger.error(e)
...
def Get_icenter(url):
    ...
    try:
        ...
        view_data = {}
    view_data.setdefault("totalVisitorCount", 0)
    view_data.setdefault("totalViewCount", 0)
...
def Icenter_children_get(url, depth=9):
    """
    获取指定iCenter页面的所有子页面链接（支持多层级）
    ...
    Returns:
        dict: 树形结构，键为markdown链接格式，值为嵌套dict或空{}
    """
    icenterapi = IcenterAPI(os.getenv('USERNAME'), os.getenv('PASSWORD'))    
    contentid, spaceid = icenterapi.get_pageid_spaceid(url)    
    result = icenterapi.get_children(spaceid, contentid, depth=depth)
...
def Icenter_page_create(url, title, content=''):
    icenterapi = IcenterAPI(os.getenv('USERNAME'), os.getenv('PASSWORD'))
    contentid, spaceid = icenterapi.get_pageid_spaceid(url)
    resp_json_data, resp_flag = icenterapi.create_page(spaceid, contentid, title, content)
    if resp_flag and resp_json_data:
        page_id = resp_json_data.get("bo", "")
        if page_id:
            page_url = f"https://i.zte.com.cn/index/ispace/#/space/{spaceid}/wiki/page/{page_id}/view"
...
def Get_icenter_page_view_count(url):
    """
    获取 iCenter 页面的浏览量和访客数据
    ...
    """
    icenterapi = IcenterAPI(os.getenv('USERNAME'), os.getenv('PASSWORD'))
    result = icenterapi.get_page_view_count(url)
    return result
...
def test_Icenter_children_get():
    import json
    result = Icenter_children_get("https://i.zte.com.cn/index/ispace/#/space/fbff14a6a14c4985874248df3ac610c1/wiki/page/e45a5803f13211f098712da37e8c0e58/view")
...
def test_Icenter_new_page():
    result = Icenter_new_page("https://i.zte.com.cn/index/ispace/#/space/c5dbe928c3f54634922abbf7012bc8c4/wiki/page/4b6480af4bb411f1bd2af36403265b9c/view", "这是一个测试页新建", "## 测试内容")
    return result


def test_Icenter_page_create():
    """测试 Icenter_page_create 函数"""
    result = Icenter_page_create(
        "https://i.zte.com.cn/index/ispace/#/space/f0fec92cefba4aa18cb7876cd6078c23/wiki/page/ee35c580ae86427f8b7e08c36b8e91d4/view",
        "AI测试-测试页面创建",
        "<h2>测试内容</h2><p>这是一个通过 test_Icenter_page_create 创建的测试页面。</p>"
    )
    return result
...
if __name__ == '__main__':
    ...
    print(test_Icenter_page_create())
