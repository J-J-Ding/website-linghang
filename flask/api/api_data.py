...
TABLE_CONFIG_MAP = {
    ...
    "FT用例库": {
        "db_path": "../data/sql_test.db",
        "table_name": "ft_testcase",
        "key_field": "用例编号",
        "fields": [
            "用例编号", "用例名称", "用例描述", "预置条件(G)", "测试步骤(W)", 
            "预期结果(T)", "领域", "团队", "组件", "特性", "功能点", 
            "创建人", "更新时间", "标签", "checked"
        ]
    },
    "AI应用库": {
...
def TOOL_Testcase_table_get(search_params):
    """
    从FT用例库获取数据，支持两种搜索方式：
    1. 模糊搜索: "LAG 删除" - 多词空格分隔，在所有字段中模糊匹配(AND关系)
    2. 指定字段搜索: {"用例编号": "20260414", "用例名称": "lag"} - 在所有字段中模糊匹配(AND关系)
    
    返回: JSON格式 (list of dict)
    
    调用示例:
        result = TOOL_Testcase_table_get("LAG")
        result = TOOL_Testcase_table_get("LAG 删除")
        result = TOOL_Testcase_table_get({"用例编号": "20260414", "用例名称": "lag"})
    """
    print(f"TOOL_Testcase_table_get: search_params = {search_params}")
    
    # 容错处理：入参为空
    if search_params is None or search_params == "" or (isinstance(search_params, dict) and not search_params):
        return []
    
    table_config = TABLE_CONFIG_MAP.get("FT用例库")
    if not table_config:
        return []
    
    db_path = table_config["db_path"]
    table_name_in_db = table_config["table_name"]
    all_fields = table_config["fields"]
    ...
    try:
        ...
        quoted_fields = [f'"{f}"' for f in all_fields]
        sql = f"SELECT {', '.join(quoted_fields)} FROM {table_name_in_db}"
        values = []
        ...
        if search_params:
            ...
            if isinstance(search_params, str):
                keywords = [k.strip() for k in search_params.split() if k.strip()]
            elif isinstance(search_params, dict):
                combined = " ".join(str(v).strip() for v in search_params.values() if v)
                keywords = [k.strip() for k in combined.split() if k.strip()]
            ...
            if keywords:
                ...
                for keyword in keywords:
                    ...
                    where_groups.append("(" + " OR ".join(field_conditions) + ")")
        ...
        print(f"TOOL_Testcase_table_get: 返回 {len(result)} 条记录")
        return result
        
    except Exception as e:
        print(f"TOOL_Testcase_table_get error: {e}")
        return []
    finally:
        if conn:
            conn.close()
...
def TOOL_Testcase_table_set(case_data):
    """
    设置FT用例库数据(新增或更新)，支持批量处理：
    - 传入用例编号: 修改指定编号的用例(用例编号必须已存在)
    - 用例编号为空: 新建用例(自动生成14位编号: YYYYMMDD + 6位自增)
    - 用例编号不存在: 返回"用例不存在"
    
    支持单条或批量处理:
    - 单条: {"用例名称": "LAG测试", "领域": "L2"}
    - 批量: [{"用例名称": "LAG测试", "领域": "L2"}, {"用例编号": "20260414000001", "用例名称": "更新"}]
    
    返回: 
    - 单条: {"status": "success/error", "message": "...", "用例编号": "..."}
    - 批量: [{"status": "success/error", "message": "...", "用例编号": "..."}, ...]
    
    调用示例:
        result = TOOL_Testcase_table_set({"用例名称": "LAG测试", "领域": "L2"})
        result = TOOL_Testcase_table_set({"用例编号": "20260414000001", "用例名称": "更新"})
        result = TOOL_Testcase_table_set([{"用例名称": "测试1"}, {"用例名称": "测试2", "领域": "L2"}])
    """
    from datetime import datetime
    
    print(f"TOOL_Testcase_table_set: case_data = {case_data}")
    
    # 容错处理：入参为空
    if case_data is None or case_data == {} or case_data == []:
        return {"status": "error", "message": "入参为空"}
    
    is_single = isinstance(case_data, dict)
    if is_single:
        case_data_list = [case_data]
    else:
        if not isinstance(case_data, list):
            return {"status": "error", "message": "输入数据无效，应为dict或list"}
        case_data_list = case_data
    ...
    table_config = TABLE_CONFIG_MAP.get("FT用例库")
    if not table_config:
        return {"status": "error", "message": "FT用例库配置不存在"}
    ...
    try:
        ...
        for case_data in case_data_list:
            if not case_data or not isinstance(case_data, dict):
                results.append({"status": "error", "message": "输入数据无效"})
                continue
            
            key_value = case_data.get(key_field)
            
            if key_value and str(key_value).strip():
                cursor.execute(f'SELECT * FROM {table_name_in_db} WHERE {quoted_key_field} = ?', (key_value,))
                ...
                if existing:
                    ...
                    for field, value in case_data.items():
                        if field in all_fields and value is not None and value != "":
                            update_fields.append(f'"{field}" = ?')
                            update_values.append(value)
                    
                    if update_fields:
                        update_sql = f"UPDATE {table_name_in_db} SET {', '.join(update_fields)} WHERE {quoted_key_field} = ?"
                        update_values.append(key_value)
                        cursor.execute(update_sql, update_values)
                        conn.commit()
                        results.append({"status": "success", "message": "更新成功", "用例编号": key_value})
                    else:
                        results.append({"status": "success", "message": "无字段需要更新", "用例编号": key_value})
                else:
                    results.append({"status": "error", "message": "用例不存在", "用例编号": key_value})
            else:
                today = datetime.now().strftime("%Y%m%d")
                
                cursor.execute(f'SELECT MAX({quoted_key_field}) FROM {table_name_in_db} WHERE {quoted_key_field} LIKE ?', (f"{today}%",))
                ...
                new_id = f"{today}{seq:06d}"
                
                case_data[key_field] = new_id
                ...
                for field in all_fields:
                    if field in case_data and case_data[field] is not None:
                        ...
                        insert_values.append(case_data[field])
                        placeholders.append("?")
                    else:
                        insert_fields.append(f'"{field}"')
                ...
                insert_sql = f"INSERT INTO {table_name_in_db} ({', '.join(insert_fields)}) VALUES ({', '.join(placeholders)})"
                cursor.execute(insert_sql, insert_values)
                conn.commit()
                results.append({"status": "success", "message": "新增成功", "用例编号": new_id})
        
        if is_single:
            result = results[0] if results else {"status": "error", "message": "处理失败"}
            print(f"TOOL_Testcase_table_set result: {result}")
            return result
        else:
            print(f"TOOL_Testcase_table_set result: {results}")
            return results
            
    except Exception as e:
        print(f"TOOL_Testcase_table_set error: {e}")
        if conn:
            conn.rollback()
        if is_single:
            return {"status": "error", "message": str(e)}
        else:
            return [{"status": "error", "message": str(e)} for _ in case_data_list]
    finally:
        if conn:
            conn.close()


def TOOL_Testcase_table_del(case_no):
    """
    删除FT用例库中指定编号的用例，支持批量删除：
    
    参数:
        case_no: 用例编号或用例编号列表
        - 单条: "20260414000001"
        - 批量: ["20260414000001", "20260414000002"]
    
    返回: 
        - 单条: {"status": "success/error", "message": "..."}
        - 批量: [{"status": "success/error", "message": "...", "用例编号": "..."}, ...]
    
    调用示例:
        result = TOOL_Testcase_table_del("20260414000001")
        result = TOOL_Testcase_table_del(["20260414000001", "20260414000002"])
    """
    print(f"TOOL_Testcase_table_del: case_no = {case_no}")
    
    # 容错处理：入参为空
    if case_no is None or case_no == "" or case_no == [] or case_no == {}:
        return {"status": "error", "message": "入参为空"}
    
    is_single = isinstance(case_no, str)
    if is_single:
        case_no_list = [case_no]
    else:
        if not isinstance(case_no, list):
            return {"status": "error", "message": "输入无效，应为字符串或列表"}
        case_no_list = case_no
    
    if not case_no_list:
        return {"status": "error", "message": "用例编号不能为空"}
    ...
    try:
        ...
        for no in case_no_list:
            if not no or not str(no).strip():
                results.append({"status": "error", "message": "用例编号不能为空", "用例编号": no})
                continue
            
            cursor.execute(f'SELECT * FROM {table_name_in_db} WHERE {quoted_key_field} = ?', (no,))
            existing = cursor.fetchone()
            
            if not existing:
                results.append({"status": "error", "message": "用例不存在", "用例编号": no})
                continue
            
            cursor.execute(f'DELETE FROM {table_name_in_db} WHERE {quoted_key_field} = ?', (no,))
            conn.commit()
            results.append({"status": "success", "message": "删除成功", "用例编号": no})
        
        if is_single:
            return results[0] if results else {"status": "error", "message": "处理失败"}
        else:
            return results
            
    except Exception as e:
        print(f"TOOL_Testcase_table_del error: {e}")
        if conn:
            conn.rollback()
        if is_single:
            return {"status": "error", "message": str(e)}
        else:
            return [{"status": "error", "message": str(e)} for _ in case_no_list]
    finally:
        if conn:
            conn.close()
...
def API_Table_set():
    ...
    try:
        ...
        valid_conditions = {k: v for k, v in conditions.items() if k in table_columns}
        invalid_fields = set(conditions.keys()) - set(table_columns)

        if 'checked' in invalid_fields:
            return jsonify({
                "status": "error",
                "message": "未支持",
                "data": []
        ...
        if invalid_fields:
            print(f"警告: 表 '{table_name}' 不支持以下字段: {invalid_fields}")
...
def _build_tree_node(title: str, page_url: str, level: int, node_type: str, max_level: int):
    ...
    for child_title, child_url in child_map.items():
        if next_level == 3:
            ...
            child_type = "category"
        children.append(_build_tree_node(child_title, child_url, next_level, child_type, max_level))
...
def API_Knowledge_component_tree():
    """
    组件树接口：返回固定二级分类 + iCenter 下钻子节点（最多4级）。
    """
    try:
        ...
        root_space_id, root_content_id = _parse_icenter_ids(root_url)
        root_title = req.get("rootTitle") or "L1领域"
        try:
            fetched_title = Icenter_title_get(root_url)
            if fetched_title:
                root_title = fetched_title
        except Exception:
            pass

        fixed_categories = [
            {"title": "L1领域-应用-组件树", "url": root_url},
        ...
        children = []
        for item in fixed_categories:
            children.append(_build_tree_node(item["title"], item["url"], 2, "category", max_level))

        root_node = {
            "id": root_content_id or "root-l1",
...
def test_TOOL_Testcase_table():
    """测试 TOOL_Testcase_table_get 和 TOOL_Testcase_table_set 函数"""
    print("\n========== FT用例库接口测试 ==========")
    
    # 测试1: 新增用例(自动生成编号)
    print("\n--- 测试1: 新增用例(自动生成编号) ---")
    result = TOOL_Testcase_table_set({
        "用例名称": "LAG删除测试",
        "领域": "L2",
        "特性": "LAG",
        "功能点": "删除",
    ...
    print(f"结果: {result}")
    new_id = result.get("用例编号")
    
    # 测试2: 根据ID查询用例
    print(f"\n--- 测试2: 查询用例编号 {new_id} ---")
    result = TOOL_Testcase_table_get(new_id)
    print(f"查询到 {len(result)} 条记录")
    if result:
        print(f"用例名称: {result[0].get('用例名称')}")
    ...
    result = TOOL_Testcase_table_get(new_id)
    if result:
        print(f"更新后用例名称: {result[0].get('用例名称')}")
    ...
    # 测试6: 删除用例
    print(f"\n--- 测试6: 删除用例 {new_id} ---")
    result = TOOL_Testcase_table_del(new_id)
    print(f"结果: {result}")
    
    # 验证删除结果
    result = TOOL_Testcase_table_get(new_id)
    print(f"删除后查询结果: {len(result)} 条记录")
    
    # 测试7: 删除不存在的用例
    print("\n--- 测试7: 删除不存在的用例 ---")
    result = TOOL_Testcase_table_del("20990101000001")
    print(f"结果: {result}")
    
    # 测试8: 批量新增用例
    print("\n--- 测试8: 批量新增用例 ---")
    result = TOOL_Testcase_table_set([
        {"用例名称": "批量测试1", "领域": "L2", "团队": "测试团队1"},
        {"用例名称": "批量测试2", "领域": "L3", "团队": "测试团队2"},
        {"用例名称": "批量测试3", "领域": "L2"}
    ])
    print(f"批量新增结果: {result}")
    batch_ids = [r.get("用例编号") for r in result if r.get("status") == "success"]
    print(f"新增的用例编号: {batch_ids}")
    
    # 测试9: 批量更新用例
    print("\n--- 测试9: 批量更新用例 ---")
    if len(batch_ids) >= 2:
        result = TOOL_Testcase_table_set([
            {"用例编号": batch_ids[0], "用例名称": "批量测试1_更新", "标签": "已更新"},
            {"用例编号": batch_ids[1], "用例名称": "批量测试2_更新"}
        ])
    ...
    print("\n--- 测试10: 批量删除用例 ---")
    if len(batch_ids) >= 3:
        result = TOOL_Testcase_table_del([batch_ids[0], batch_ids[1], batch_ids[2]])
    ...
    if len(batch_ids) >= 1:
        result = TOOL_Testcase_table_set([
            {"用例名称": "混合测试1"},  # 新增
            {"用例名称": "混合测试2"},  # 新增
            {"用例编号": batch_ids[-1], "用例名称": "混合测试3_更新"}  # 更新
        ])
        print(f"混合批量结果: {result}")
    
    # 测试12: 批量删除不存在的用例
    print("\n--- 测试12: 批量删除不存在的用例 ---")
    result = TOOL_Testcase_table_del(["20990101000001", "20990101000002"])
    print(f"结果: {result}")
    
    print("\n========== FT用例库接口测试完成 ==========")

def API_Table_update_stats():
    """
    API 接口：获取指定表的更新统计数据
    统计最近1天、7天、30天的更新次数
    ...
    """
    print("API_Table_update_stats: 接收到获取更新统计的请求")
    ...
    db_path = "../data/sql_record.db"
    table_name_in_db = "table_update_record"
    ...
    try:
        ...
        sql = f"""
        SELECT
            COUNT(CASE WHEN update_time >= datetime('now', '-1 day') THEN 1 END) as days1,
            COUNT(CASE WHEN update_time >= datetime('now', '-7 day') THEN 1 END) as days7,
            COUNT(CASE WHEN update_time >= datetime('now', '-30 day') THEN 1 END) as days30
        FROM {table_name_in_db}
        WHERE table_name = ?
        """
        ...
        if row:
            ...
            result = {"days1": 0, "days7": 0, "days30": 0}
        ...
        print(f"API_Table_update_stats: 数据库错误 - {e}")
        return jsonify({
            "status": "error",
        ...
        print(f"API_Table_update_stats: 错误 - {e}")
        return jsonify({
            "status": "error",
...
if __name__ == "__main__":
    ...
    # test_TOOL_Table_get()
    test_TOOL_Testcase_table()  # 取消注释以运行测试
